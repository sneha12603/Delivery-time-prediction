    ...
    return (
        ...
        .assign(
            ...
            order_time = lambda x: pd.to_datetime(
                x['order_time'].replace("NaN ", np.nan), errors='coerce'
                ),
            order_picked_time = lambda x: pd.to_datetime(
                x['order_picked_time'].replace("NaN ", np.nan), errors='coerce'
                 ),
...
def extract_datetime_features(ser: pd.Series) -> pd.DataFrame:
    data_col = pd.to_datetime(ser,dayfirst=True)
