...
if __name__ == "__main__":
    ...
    logger.info("Ranodm Forest model built")

    #build random forst model
    lgbm_params = model_params["LightGBM"]
    logger.info("LightGBM parameters read")
    lgbm = LGBMRegressor(**lgbm_params)
    logger.info("build the LightGBM model")
    ...
    stacking_reg = StackingRegressor(estimators=[("rf_model",rf),("lgbm_model",lgbm)],
    ...
    logger.info("Model wraperd inside wrapper")
    ...
    #extract the model from wrapeer
    stacking_model = model.regressor_
    transformer = model.transformer

    #save the model
    save_model(model=model,save_dir=model_save_dir,
                model_name=model_filename)
    logger.info("Stacking model saved to location")
